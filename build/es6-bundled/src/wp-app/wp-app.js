...
resolved=(location.origin||location.protocol+"//"+location.host)+resolved}return resolved}});var Utility={distance:function(x1,y1,x2,y2){var xDelta=x1-x2,yDelta=y1-y2;return Math.sqrt(xDelta*xDelta+yDelta*yDelta)},now:window.performance&&window.performance.now?window.performance.now.bind(window.performance):Date.now};/**
    * @param {HTMLElement} element
    * @constructor
...
if(!label||oldValue==label){this.setAttribute("aria-label",newValue)}}});class WpApp extends PolymerElement{static get template(){return html`
      <style >
        :host {
